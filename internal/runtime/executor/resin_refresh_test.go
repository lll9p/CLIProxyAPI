...
func TestOAuthRefreshRoutesThroughResin(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			provider: "codex",
			expectedPaths: []string{
				"/secret/cpa/https/auth.openai.com/oauth/token",
			},
		...
		},
		{
			...
			provider: "claude",
			expectedPaths: []string{
				"/secret/cpa/https/platform.claude.com/v1/oauth/token",
				"/secret/cpa/https/api.anthropic.com/api/oauth/profile",
			},
		...
		},
		{
			...
			provider: "kimi",
			expectedPaths: []string{
				"/secret/cpa/https/auth.kimi.com/api/oauth/token",
			},
		...
		},
		{
			...
			provider: "kimi-ai",
			expectedPaths: []string{
				"/secret/cpa/https/auth.kimi.ai/api/oauth/token",
			},
			response: `{"access_token":"kimi-ai-access","refresh_token":"kimi-ai-refresh-new","expires_in":3600}`,
			metadata: map[string]any{
				"type":          "kimi-ai",
				"domain":        "kimi.ai",
				"refresh_token": "kimi-ai-refresh",
				"device_id":     "device-ai-1",
			},
			refresh: func(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
				return NewKimiExecutor(cfg).Refresh(ctx, auth)
			},
		},
		{
			...
			provider: "xai",
			expectedPaths: []string{
				"/secret/cpa/https/auth.x.ai/oauth/token",
			},
		...
		},
		{
			...
			provider: "meta",
			expectedPaths: []string{
				"/secret/cpa/https/api.meta.ai/muse-code/key",
			},
			response: `{"api_key":"meta-access"}`,
			metadata: map[string]any{
				"auth_kind": "oauth",
				"dca_token": "dca:meta-refresh",
			},
			refresh: func(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
				return NewMetaExecutor(cfg).Refresh(ctx, auth)
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			resinServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
				...
				if !allowed {
					t.Errorf("Resin path = %q, want one of %v", req.URL.EscapedPath(), test.expectedPaths)
				}
				if got := req.Header.Get("X-Resin-Account"); got != test.provider+"-user.json" {
					t.Errorf("X-Resin-Account = %q", got)
				}
				w.Header().Set("Content-Type", "application/json")
				if strings.HasSuffix(req.URL.EscapedPath(), "/api/oauth/profile") {
					_, _ = w.Write([]byte(`{"account":{"uuid":"user-uuid","email":"user@example.com"},"organization":{"uuid":"org-uuid","name":"Org"}}`))
					return
				}
