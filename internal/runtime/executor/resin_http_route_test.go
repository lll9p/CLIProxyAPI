...
func TestGenericHTTPRequestsRouteThroughResin(t *testing.T) {
	...
	resinServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		records <- resinRouteRecord{
			path:           req.URL.EscapedPath(),
			account:        req.Header.Get("X-Resin-Account"),
	...
	tests := []struct {
		...
		{name: "kimi-ai", provider: "kimi-ai", request: NewKimiExecutor(cfg).HttpRequest},
		{name: "kimi.ai", provider: "kimi.ai", request: NewKimiExecutor(cfg).HttpRequest},
		{name: "kimi.com", provider: "kimi.com", request: NewKimiExecutor(cfg).HttpRequest},
		...
		{name: "devin", provider: "devin", wantAcceptEncoding: "identity", request: NewDevinExecutor(cfg).HttpRequest},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			ctx := cliproxyexecutor.WithRequestProxyURL(context.Background(), "http://127.0.0.1:1")
			resp, errDo := test.request(ctx, auth, req)
			if errDo != nil {
			...
			case record := <-records:
				...
				if test.wantAcceptEncoding != "" && record.acceptEncoding != test.wantAcceptEncoding {
					t.Fatalf("Accept-Encoding = %q, want %q", record.acceptEncoding, test.wantAcceptEncoding)
				}
			default:
