...
func TestXAIDialWebsocketUsesDirectDialerForResin(t *testing.T) {
	...
	_, closer, resp, errDial := exec.dialXAIWebsocket(ctx, auth, "wss://api.x.ai/v1/responses", headers)
	if errDial != nil {
...
func TestXAIEnsureUpstreamConnReusesMatchingDialKeyAndReconnectsOnChange(t *testing.T) {
	...
	if closer1 == nil {
		t.Fatal("first xAI websocket closer is nil")
	}
	...
	if closer2 != closer1 {
		t.Fatal("matching dial key did not reuse the xAI websocket closer")
	}
	select {
	case <-accepted:
	...
	if closer3 == nil {
		t.Fatal("reconnected xAI websocket closer is nil")
	}
