...
func TestCodexDialWebsocketUsesDirectDialerForResin(t *testing.T) {
	...
	ctx := cliproxyexecutor.WithRequestProxyURL(context.Background(), "http://127.0.0.1:3")
	_, closer, resp, errDial := exec.dialCodexWebsocket(ctx, auth, "wss://chatgpt.com/backend-api/codex/responses", headers)
	if errDial != nil {
...
func TestCodexEnsureUpstreamConnReusesMatchingDialKeyAndReconnectsOnChange(t *testing.T) {
	...
	conn1, closer1, resp1, errFirst := exec.ensureUpstreamConn(context.Background(), auth, sess, auth.ID, wsURL, headers)
	if errFirst != nil {
		t.Fatalf("first ensureUpstreamConn() error = %v", errFirst)
	}
	closeHTTPResponseBody(resp1, "close first Codex handshake response")
	if closer1 == nil {
		t.Fatal("first Codex websocket closer is nil")
	}
	waitForWebsocketConnection(t, accepted, "first Codex websocket connection")

	conn2, closer2, resp2, errSecond := exec.ensureUpstreamConn(context.Background(), auth, sess, auth.ID, wsURL, headers)
	if errSecond != nil {
	...
	if closer2 != closer1 {
		t.Fatal("matching dial key did not reuse the Codex websocket closer")
	}
	select {
	case <-accepted:
	...
	conn3, closer3, resp3, errThird := exec.ensureUpstreamConn(context.Background(), auth, sess, auth.ID, wsURL, changedHeaders)
	if errThird != nil {
	...
	if closer3 == nil {
		t.Fatal("reconnected Codex websocket closer is nil")
	}
