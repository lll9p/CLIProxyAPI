...
func (h *Handler) apiCallOutboundTransport(auth *coreauth.Auth, requestProxyURL string) http.RoundTripper {
	var cfg *config.Config
	if h != nil {
		cfg = h.cfg
	}
	return resin.WrapTransport(cfg, auth, h.apiCallTransport(auth, requestProxyURL))
}
