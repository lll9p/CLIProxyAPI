...
func TestResolveXAITokenRoutesRefreshThroughResin(t *testing.T) {
	var upstreamHits atomic.Int32
	upstreamServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		upstreamHits.Add(1)
	...
	resinServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		if got, want := req.URL.EscapedPath(), "/secret/cpa/http/"+upstreamHost+"/oauth/token"; got != want {
			t.Errorf("Resin path = %q, want %q", got, want)
		}
		if got, want := req.Header.Get("X-Resin-Account"), "xai-user.json"; got != want {
			t.Errorf("X-Resin-Account = %q, want %q", got, want)
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"access_token":"fresh-token","refresh_token":"fresh-refresh","token_type":"Bearer","expires_in":3600}`))
	}))
	defer resinServer.Close()
	...
	auth := &coreauth.Auth{
		...
		Metadata: map[string]any{
			...
			"token_endpoint": upstreamServer.URL + "/oauth/token",
			"expired":        time.Now().Add(-time.Hour).Format(time.RFC3339),
		},
	}

	token, err := h.resolveXAIToken(context.Background(), auth, proxyServer.URL)
	if err != nil {
		t.Fatalf("resolveXAIToken() error = %v", err)
	}
	if token != "fresh-token" {
		t.Fatalf("resolveXAIToken() = %q, want fresh-token", token)
	}
	if got := upstreamHits.Load(); got != 0 {
