...
func TestNewDeviceFlowClientWithDeviceIDAndHTTPClient(t *testing.T) {
	...
	aiClient := NewDeviceFlowClientWithDomainDeviceIDAndHTTPClient(KimiAIDomain, "device-ai", injected)
	if got := aiClient.tokenURL(); got != "https://auth.kimi.ai/api/oauth/token" {
		t.Fatalf("Kimi.ai token URL = %q", got)
	}
}
