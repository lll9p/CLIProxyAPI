...
func TestRefreshTokens_DoesNotDeduplicateDifferentRouteAccounts(t *testing.T) {
	...
	transport := roundTripFunc(func(req *http.Request) (*http.Response, error) {
		// Upstream RefreshTokens enriches the token data with an OAuth profile
		// lookup after a successful refresh; only refresh POSTs must be counted.
		if req.Method == http.MethodGet && req.URL.Path == "/api/oauth/profile" {
			return &http.Response{
				StatusCode: http.StatusOK,
				Body:       io.NopCloser(strings.NewReader(`{"account":{"uuid":"u","email":"e@x"},"organization":{"uuid":"o","name":"n"}}`)),
