...
func TestSupportsAuth(t *testing.T) {
	for _, provider := range []string{"codex", "claude", "antigravity", "gemini", "gemini-interactions", "kimi", "kimi-ai", "kimi.ai", "kimi.com", "xai", "vertex", "devin", "meta"} {
	...
	for _, provider := range []string{"vertex", "meta"} {
		apiKey := oauthAuth(provider, provider+".json")
		apiKey.Attributes[cliproxyauth.AttributeAuthKind] = cliproxyauth.AuthKindAPIKey
		if supportsAuth(apiKey) {
			t.Fatalf("supportsAuth(%s API key) = true, want false", provider)
		}
	}
	if supportsAuth(nil) {
...
func TestPrepareWebSocketDialKeyStabilityAndInputs(t *testing.T) {
	...
	for _, name := range []string{"X-Codex-Routing-Hint", "Session_id"} {
		withVolatile := cloneHeadersWith(headers, name, "request-specific")
		_, _, key, _ := PrepareWebSocket(cfg, auth, target, withVolatile)
		if key != baseKey {
			t.Fatalf("per-request %s changed connection identity", name)
		}
	}
