...
func supportsAuth(auth *cliproxyauth.Auth) bool {
	...
	case "codex", "claude", "antigravity", "gemini", "gemini-interactions", "kimi", "kimi-ai", "kimi.ai", "kimi.com", "xai", "vertex", "devin", "meta":
		return true
	default:
		return false
	}
...
func identityHeadersDigest(headers http.Header) string {
	...
	for name, headerValues := range headers {
		// Codex regenerates these per request; neither changes the authenticated account.
		if strings.EqualFold(name, "X-Codex-Routing-Hint") || strings.EqualFold(name, "Session_id") {
			continue
		}
